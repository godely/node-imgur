 ...
 * @param   {string}  id - deletehash of the image file
 * @returns {promise}
 */
imgur.deleteImage = function (id) {
    var deferred = Q.defer();

    imgur._imgurRequest('delete', id)
        .then(function (json) {
            deferred.resolve(json);
        })
        .catch(function (error) {
 ...
 * @returns {promise}
 */
imgur.updateImage = function (id, config) {
    var deferred = Q.defer(),
        extraFormParams = {};

    if (config.hasOwnProperty('title')) {
        extraFormParams.title = config.title;
    }

    if (config.hasOwnProperty('description')) {
        extraFormParams.description = config.description;
    }
 ...
 * @returns {promise}
 */
imgur.uploadFile = function (file, config) {
    var deferred = Q.defer(),
        extraFormParams = {};

    if (config.hasOwnProperty('album')) {
        extraFormParams.album = config.album;
    }
